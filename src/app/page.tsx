...
const issuerLogo: Record<string, { type: 'img'; src: string } | { type: 'badge'; color: string; bg: string; text: string }> = {
  'Amazon Web Services (AWS)': { type: 'img', src: 'https://api.iconify.design/logos:aws.svg' },
  'HashiCorp':                 { type: 'img', src: 'https://api.iconify.design/file-icons:hashicorp.svg' },
  'dbt Labs':                  { type: 'img', src: 'https://api.iconify.design/logos:dbt.svg' },
  'Astronomer':                { type: 'img', src: 'https://api.iconify.design/logos:astronomer.svg' },
  'Scrum.org':                 { type: 'badge', color: '#fff', bg: '#009FDA', text: 'S' },
};

function IssuerLogo({ issuer }: { issuer: string }) {
  const logo = issuerLogo[issuer];
  if (!logo) return <span className="w-5 h-5 rounded-sm shrink-0 inline-block bg-[#d1d1d1]" />;
  if (logo.type === 'img') {
    return <img src={logo.src} alt={issuer} className="w-5 h-5 shrink-0 object-contain" />;
  }
  return (
    <span
      className="h-5 min-w-5 rounded-sm shrink-0 inline-flex items-center justify-center text-[9px] font-bold px-1"
...
export default function Home() {
  return (
    <div className="min-h-screen bg-white text-[#1a1a1a]">
      ...
      <Link
        href="/terminal"
        className="print:hidden fixed top-5 left-5 z-50 flex items-center gap-2 px-4 py-2 rounded-full border border-[#d1d1d1] bg-white text-[#1a1a1a] text-sm font-mono no-underline hover:border-[#999] transition-colors shadow-sm"
      >
      ...
      <main className="max-w-2xl mx-auto px-8 py-20">
        ...
        <header className="text-center mb-16">
          ...
          <div className="flex justify-center gap-3">
            ...
            >
              ...
              <span className="text-xs font-mono whitespace-nowrap">+34 684 021 763</span>
            </a>
            <a
              href="mailto:carlosru0022@gmail.com"
              className="flex items-center justify-center gap-2 px-3 h-11 border border-[#d1d1d1] rounded-lg no-underline hover:border-[#999] transition-colors"
              title="Gmail"
            >
              ...
              <span className="hidden print:inline text-xs font-mono text-[#444]">carlosru0022@gmail.com</span>
            </a>
            <a
            ...
            >
              ...
              <span className="hidden print:inline text-xs font-mono text-[#444]">linkedin.com/in/carlos-ruiz-torres</span>
            </a>
            <a
              ...
              title="GitHub"
            >
              <svg viewBox="0 0 24 24" className="w-5 h-5 shrink-0" fill="#1a1a1a" xmlns="http://www.w3.org/2000/svg">
              ...
              <span className="hidden print:inline text-xs font-mono text-[#444]">github.com/carlosru99</span>
            </a>
          </div>
        </header>
        ...
        {/* ── Certifications ── */}
        <section className="mb-12">
          <h2 className="text-2xl font-bold text-[#1a1a1a] mb-5">Certifications</h2>
              ...
              <div key={i} className="flex items-center gap-2">
                ...
                {cert.url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="font-mono text-sm text-[#1a1a1a] hover:underline leading-snug"
                  >
                    {cert.name}
                  </a>
                ) : (
                  <span className="font-mono text-sm text-[#1a1a1a] leading-snug">{cert.name}</span>
                )}
              </div>
            ))}
          </div>
